...
LEFT JOIN (
	SELECT
		...
		ref_cyl.CH4,
		ref_cyl.sdCH4
	FROM RefGasCylinder_Deployment AS cyl
	JOIN RefGasCylinder AS ref_cyl
	ON cyl.CylinderID = ref_cyl.CylinderID
	AND (cyl.Date_UTC_from >= ref_cyl.Date_UTC_from AND cyl.Date_UTC_to <= ref_cyl.Date_UTC_to)
) AS ref_gas
ON cal.LocationName = ref_gas.LocationName
AND cal.SensorUnit_ID = ref_gas.SensorUnit_ID
AND cal.Date_UTC_from <= ref_gas.Date_UTC_from
JOIN Sensors AS sens
	ON sens.SensorUnit_ID = cal.SensorUnit_ID AND
	(sens.Date_UTC_from  <= cal.Date_UTC_from AND  sens.Date_UTC_to >= sens.Date_UTC_to)
	AND Type = 'HPP'
WHERE CalMode IN (1,2,3) 
AND cal.SensorUnit_ID IN ({sens*})"


conn <- carboutil::get_conn(user='basi')
cal_data <- carboutil::get_query_parametrized(conn, cal_query, sens = SensorUnit_ID_2_cal)
