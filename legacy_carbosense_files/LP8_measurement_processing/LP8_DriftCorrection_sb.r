...
# -----------------------------------------------------
#
# Author: Simone Baffelli. Based on Michaels Müller Script
#
# -----------------------------------------------------
#
# Remarks:
...
#Overload dBFetch to respect UTC as timezone
dbFetch <- function(...){
    lubridate::with_tz(DBI::dbFetch(...), tzone="UTC")
}

## PARAMETERS

apply_SU_events <- T
project_dir <- "/project/CarboSense/Carbosense_Network"

excluded_locations <- c('DUE2', 'DUE3', 'DUE4', 'DUE5', 'DUE6', 'MET1')
...
parser$add_argument("model", choices=c(1,2,3), type="integer", help="Model type")
parser$add_argument("partial", type="character", default="F", help="Partial processing or complete processing")
parser$add_argument("DUE", type="character", help="Process only DUE1 data?", default="F", nargs="?")
parser$add_argument("SensorUnit_ID", nargs='?', type="integer", help="Process only given sensor unit.")
args_in <- commandArgs(trailingOnly=TRUE)
args <- parser$parse_args(args_in)
args$DUE <- as.logical(args$DUE)
args$partial <- as.logical(args$partial)
#Determine table and folders depending on input
ProcMeasDBTableName <- switch(args$model, "CarboSense_CO2", "CarboSense_CO2_TEST00","CarboSense_CO2_TEST01")
resultdir_postfix <- switch(args$model, "LP8_PerformanceAnalysis", "LP8_PerformanceAnalysis_TEST00", "LP8_PerformanceAnalysis_TEST00")
resultdir <- file.path(project_dir, resultdir_postfix)
anchor_events_fn   <- file.path(resultdir,"anchor_events.csv") 
SU_events_fn   <- ifelse(args$DUE, file.path(resultdir,"SU_events_DUE1.csv"), file.path(resultdir,"SU_events.csv"))
...
if(args$partial){
    PARTIAL_COMPUTING <- T
    nd <- 90
    date_end <- lubridate::now("UTC")
    date_start <- date_end - lubridate::days(x=nd)

}else{
    PARTIAL_COMPUTING <- F
    date_start <- lubridate::as_datetime("20170701000000")
    date_end <- lubridate::as_datetime("21000101000000")
}

min_time <- as.numeric(date_start)
max_time <- as.numeric(date_end)
REF_data_timestamp_from             <- min_time
REF_data_timestamp_to               <- max_time
SENSOR_data_ANALYSE_timestamp_from  <- min_time
SENSOR_data_ANALYSE_timestamp_to    <- max_time
SENSOR_data_DBINSERT_timestamp_from <- min_time
SENSOR_data_DBINSERT_timestamp_to   <- max_time
...
print("Getting reference site coordinates")

ref_sites_query <- 
"
SELECT
	(CASE WHEN loc.LocationName='DUE1' THEN 'DUE' ELSE loc.LocationName END) AS name, 
	Y_LV03 AS x,
	X_LV03 AS y,
	h,
	Canton AS canton,
	loc.Date_UTC_from,
	loc.Date_UTC_to,
    UNIX_TIMESTAMP(Date_UTC_from) AS timestamp_from,
    UNIX_TIMESTAMP(LEAST(Date_UTC_to, CURDATE())) AS timestamp_to
...
(	
    ...
    AND dep.LocationName NOT IN ({excluded_locations*})
)
OR LocationName IN ('PAY', 'RIG', 'HAE', 'GIMM', 'LAEG','BRM')
"
con <-carboutil::get_conn(group="CarboSense_MySQL")
ref_sites_query_interp <-  glue::glue_sql(ref_sites_query, .con=con)
REF_SITES <- mutate(collect(tbl(con,sql(ref_sites_query_interp))), status=1)
#Add DUEBSL site
DUEBSL <- filter(REF_SITES, name=='DUE1') %>%
mutate(name="DUE",h=432, canton="ZH", status=1)
REF_SITES <- bind_rows(REF_SITES, DUEBSL)


print(paste("The active reference sites are:", paste(REF_SITES$name[REF_SITES$status==1], collapse=",")))
...
print("Preparing deployment table")
...
SELECT 
    *,
    ...
    UNIX_TIMESTAMP(LEAST(Date_UTC_to, CURDATE())) AS timestamp_to
FROM Deployment 
WHERE SensorUnit_ID BETWEEN 1010 and 1334
"

drv             <- dbDriver("MySQL")
con<-carboutil::get_conn(group="CarboSense_MySQL")
res             <- dbSendQuery(con, dep_query)
tbl_deployment  <- dbFetch(res, n=-1)

if(!COMP_DUE){
    tbl_deployment <- filter(tbl_deployment, !(LocationName %in% excluded_locations))
}
if(COMP_DUE){
  tbl_deployment <- filter(tbl_deployment, LocationName == 'DUE1')
}
...
con <- carboutil::get_conn(group="CarboSense_MySQL")
query_str        <- glue::glue_sql("SELECT * FROM Location WHERE LocationName IN ({unique(tbl_deployment$LocationName)*});", .con=con)
...
query_str        <-  "SELECT * FROM Location WHERE Network = 'METEOSWISS';"
drv              <- dbDriver("MySQL")
con<-carboutil::get_conn(group="CarboSense_MySQL")
res              <- dbSendQuery(con, query_str)
...
print("Starting to import meteo measurements")
...
wind_query <-
"
SELECT LocationName, timestamp,Winddirection AS WDIR, Windspeed AS WSPEED, Sunshine AS SUNSHINE FROM METEOSWISS_Measurements
WHERE timestamp BETWEEN {REF_data_timestamp_from} AND {REF_data_timestamp_to} AND
LocationName IN
(
  SELECT  
  LocationName
  ...
  HAVING MAX(Windspeed) > -999
)
"
if(T){


wind_query_interp <- glue::glue_sql(wind_query)
print(wind_query_interp)
MCH_WIND_LONG <- collect(tbl(con,sql(wind_query_interp))) 
dbDisconnect(con)

print("Obtained wind measurements")
...
print("Importing Picarro data")
pic_query_str <-
"WITH DUE AS(
    SELECT 'DUE' AS Location, timestamp, CO2 AS CO2, CO2_F * H2O_F AS CO2_F FROM NABEL_DUE WHERE CO2 != -999 and H2O != -999 and H2O_F=1  AND timestamp >= {REF_data_timestamp_from} AND timestamp < {REF_data_timestamp_to}
  ),
  PAY AS(
  ...
  SELECT
    ...
    AVG(CASE WHEN CO2_F = 0 THEN NULL ELSE CO2 END) AS CO2,
    1 AS CO2_F
  FROM DUE
  GROUP BY timestamp DIV 600
  UNION ALL
  SELECT
    'PAY' AS Location,
    MIN(timestamp) AS timestamp, 
...
rename_fun<- function(x) sub("(CO2|CO2_F)_(LAEG|DUE|HAE|BRM|GIMM|PAY|RIG)","\\2_\\1", x)
...
rename_at(vars(-timestamp), rename_fun) %>%
mutate(date = lubridate::as_datetime(timestamp, tz="UTC"))
...
{
    sn <- select(data, matches("*_CO2$")) %>% colnames()
    dm <- select(data, matches("*_CO2$")) %>% data.matrix() %>%
    apply(1, function(x) replace(x, is.na(x), mean(x, na.rm = TRUE))) %>%
    t()
    dm_sv <- svd(dm)
    fd <- dm_sv$d * 0
    fd[1:3] <- 1
    dm_sv$u %*% diag(dm_sv$d * fd) %*% t(dm_sv$v)

    as.data.frame(dm[,1]) %>%
    set_names(c("CO2")) %>%
    bind_cols(date=data$date)
}



dm <- create_baseline(data)
print("Done importing and preparing Picarro data")
gc()
...
tmp              <- timeAverage(mydata = data.frame(date=dm$date,CO2=dm$CO2,stringsAsFactors = F),
                                avg.time = "week",
                                data.thresh = 50,
...
#Determine the first date to start averaging (needed for OpenAir::timeAverage)
con <- carboutil::get_conn(group="CarboSense_MySQL")
min_ts_query <- "SELECT min(timestamp) AS ts FROM CarboSense_HPP_CO2  WHERE SensorUnit_ID BETWEEN 426 and 445 and LocationName NOT IN ({excluded_locations*}) AND timestamp BETWEEN {REF_data_timestamp_from} AND {REF_data_timestamp_to}"
min_ts_query_interp <- glue::glue_sql(min_ts_query, .con=con)
min_ts <- collect(tbl(con, sql(min_ts_query_interp)))$ts
dbDisconnect(con)
avg_start_date <-  strftime(lubridate::as_datetime(min_ts, tz="UTC"),"%Y-%m-%d %H:00:00")
...
HPP_data_query <-  glue::glue_sql("
                    SELECT 
                      ...
                      AVG(CO2_CAL_ADJ) AS CO2
                    FROM CarboSense_HPP_CO2 
                    WHERE LocationName NOT IN ({excluded_locations*})
                    AND timestamp BETWEEN {REF_data_timestamp_from} AND {REF_data_timestamp_to} AND Valve = 0 AND CO2_CAL_ADJ <> -999
                    GROUP BY LocationName, SensorUnit_ID, timestamp DIV 600
                    ", .con=con)
HPP_data <- collect(tbl(con, sql(HPP_data_query)))


# print(avg_start_date)
# print(tmp)
# HPP_data <- group_by(tmp, LocationName, SensorUnit_ID) %>%
#   group_modify(
#     function(x,y) timeAverage(x ,avg.time = "10 min",statistic="mean", start_date = avg_start_date)
#   ) %>%
#   mutate(date=lubridate::as_datetime(timestamp, tz="UTC"))
...
# HPP_data$timestamp <- as.numeric(HPP_data$date)
# #

print(data[,c("date","timestamp")])

HPP_data <- tidyr::pivot_wider(HPP_data ,
id_col=c(timestamp, date), 
names_from=c(LocationName, SensorUnit_ID), 
values_from=c(CO2), 
names_prefix="HPP_") %>%
rename_at(vars(-timestamp,-date), function(x) paste0(x, "_CO2"))
...
data   <- merge(data,HPP_data,all=T,by=c("timestamp"))
...
save.image(file=file.path(project_dir,"PROCESSING","state.RData"))
...
if(T){
  ...
  for(SU_id in 1010:1334){
    ...
    for(ith_depl in 1:n_id_depl){
      ...
      print(paste("Correcting deployment",paste(tbl_deployment[id_depl[ith_depl],], collapse=",")))
      ...
      for(ith_uAENO in 1:n_u_ADJUSTMENT_EVENT_NO){
        ...
        for(ith_id_REF_selected in 1:length(id_REF_selected)){
          ...
          if(REF_site_selected=="DUEBSL" || is.na(REF_site_selected)){
            ...
            qs <- "(HPP_)?{nm}((_\\d+_)?|_)CO2$"
            gp <- glue::glue(qs, nm=REF_site_selected)
            #gp <- paste(REF_site_selected,"(HPP_)[[:print:]]{1,6}","CO2$",sep="")
            pos_REF_CO2 <- grep(pattern = gp,x = colnames(data))
          }

          if(length(pos_REF_CO2)==0){
            pos_REF_CO2 <- which(colnames(data)==paste("DUE_CO2_BSL",sep=""))
          }
        ...
        print(paste("For: ", paste(tbl_deployment[id_depl[ith_depl],], collapse=","), ":", paste(CO2_REF_sd, collapse=",")))
        print("Preparing anchor events")
        anchor_events_SU_ALL <- rbind(anchor_events_SU_ALL,data.frame(SensorUnit_ID  = tbl_deployment$SensorUnit_ID[id_depl[ith_depl]],
                                                                      ...
                                                                      timestamp_from = as.numeric(min(data$date[id_WIND]),units="secs"),
                                                                      timestamp_to   = as.numeric(max(data$date[id_WIND]),units="secs"),
...
if(!COMP_DUE){
    write.table(x = anchor_events_SU_ALL,file = file.path(resultdir,"anchor_events_SU_ALL.csv"),col.names = T,row.names = F,sep=";")
  }
  if(COMP_DUE){
    write.table(x = anchor_events_SU_ALL,file = file.path(resultdir,"anchor_events_SU_ALL_DUE1.csv"),col.names = T,row.names = F,sep=";")
  }
...
### ----------------------------------------------------------------------------------------------------------------------------
anchor_events_SU_ALL <- read.table(file.path(resultdir,"anchor_events_SU_ALL.csv"),sep=";",header=T,as.is=T)
...
figname <- file.path(resultdir,"CO2_adjustment_ZH.pdf")
corr_plt <- ggplot(corr, aes(x=(Date_UTC_from + Date_UTC_to)/2, y=SU_CO2_corr, group=SensorUnit_ID, color=SensorUnit_ID)) + geom_line()
ggsave(plot=corr_plt, file=figname, width=12, height=8)
...
# xaxis_dates     <- seq(date_start, date_end,"month") 
...
if(T){
  ...
  figname <- file.path(resultdir,"CO2_adjustment_ZH_PIC01.pdf")
  ...
  figname <- file.path(resultdir,"CO2_adjustment_ZH_PIC02.pdf")
...
for(mode in 2:1){
  ...
  figname <- paste0(resultdir,"/DIFF_CO2_TESTSITE_REFSITE_HIST_MODE_",sprintf("%02.0f",mode),".pdf")
...
write.table(x = ProofOfConcept_statistics,file = file.path(resultdir, "ProofOfConcept_statistics.csv"),col.names = T,row.names = F,sep=";")
...
write.table(x = adjustment_statistics,file = file.path(resultdir,"adjustment_statistics.csv"),col.names = T,row.names = F,sep=";")
...
figname <- file.path(resultdir,"SU_CORRECTIONS.pdf")
...
write.table(x = SU_CORRECTIONS_LARGE,file = file.path(resultdir,"SU_CORRECTIONS_LARGE.csv"),col.names = T,row.names = F,sep=";")
