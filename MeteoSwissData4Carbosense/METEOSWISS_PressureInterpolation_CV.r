...
Date_UTC_start  <- strptime("20170701000000","%Y%m%d%H%M%S",tz="UTC")
# Date_UTC_start  <- strptime("20181230000000","%Y%m%d%H%M%S",tz="UTC")
Date_UTC_end    <- strptime("20190101000000","%Y%m%d%H%M%S",tz="UTC")
...
while(timestamp_now<timestamp_end){
  ...
  if(DO_CROSSVALIDATION==T){
    ...
    print(paste(timestamp_now,max(PressurePedictions$timestamp)))
  ...
  if(F & timestamp_now >= max(PressurePedictions$timestamp)){
  ...
  if(n_id_ok>10){
    ...
    for(ii in 0:n_u_MCH_sites){
      ...
      if(n_id_ok>10){
        ...
        hist(MCH_CV_data$pred_pressure[id_ok]-MCH_CV_data$pressure[id_ok],seq(-200,200,0.5),xlim=c(-10,10),col="slategray",main=mainString,xlab="Interpolated pressure - measured pressure [hPa]",cex.axis=1.5,cex.lab=1.5,cex.main=1.5)

        par(family="mono")
        legend("topright",legend=c(str00,str01,str02,str03,str04,str05,str06,str07,str08),bg="white",cex=1.25)
        par(family="")
      }
    }
